...
if printf '%s\n' "$NORM" | grep -qE '^(review (the |this )?pr|run (the )?review|trigger (the )?review|adversarial review|reviewers?\??)[.?!]?$'; then
  if has_slot pr_review_skill; then
    ...
    REVIEWER_ROSTER=$(slot reviewer_roster "your configured reviewer agents")
    CTX="${CTX}INTENT — PR REVIEW (skill routing). REQUIRED: your next tool call MUST be Skill(skill='${PR_REVIEW_SKILL}'), passing the PR number as args if named. The skill spawns specialized reviewer agents in parallel (${REVIEWER_ROSTER}), applies the mechanical comment-style + privacy pre-gate, enforces the verification gate, and validates the dismissal log. Doing review manually skips the verification gate and dismissal-log discipline. Exception: if the user's ask is narrower (e.g., 'is the diff sane?'), surface and confirm.
"
  else
    CTX="${CTX}INTENT — PR REVIEW. Route to your PR-review skill/agents if configured. If not, fan out to independent reviewer passes covering at minimum correctness, security, and any domain-specific risk the diff touches; apply a mechanical comment-style + privacy pre-gate before presenting findings; require every dismissed finding to carry demonstrable evidence, not assertion. A single undifferentiated pass skips the independent-perspective and dismissal-log discipline. Exception: if the user's ask is narrower (e.g., 'is the diff sane?'), surface and confirm.
"
  fi
fi
